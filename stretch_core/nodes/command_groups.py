...
class HeadPanCommandGroup(SimpleCommandGroup):
    def __init__(
        self,
        range_rad=None,
        calibrated_offset_rad=None,
        calibrated_looked_left_offset_rad=None,
        node=None,
    ):
        SimpleCommandGroup.__init__(
            self, "joint_head_pan", range_rad, acceptable_joint_error=0.15, node=node
        )
        ...
        if calibrated_looked_left_offset_rad is None:
            calibrated_looked_left_offset_rad = node.controller_parameters[
    ...
    def update_joint_range(self, joint_range, node=None):
        ...
        range_rad = (
            node.robot.head.motors["head_pan"].ticks_to_world_rad(range_ticks[1]),
    ...
    def update_execution(self, robot_status, **kwargs):
        ...
        if self.active:
            pan_backlash_correction = (
                self.looked_left_offset_rad if self.looked_left else 0.0
            )
            pan_current = (
                robot_status["head"]["head_pan"]["pos"]
                + self.calibrated_offset_rad
                + pan_backlash_correction
            )
...
class HeadTiltCommandGroup(SimpleCommandGroup):
    def __init__(
        ...
        range_rad=None,
        calibrated_offset_rad=None,
        calibrated_looking_up_offset_rad=None,
        backlash_transition_angle_rad=None,
        node=None,
    ):
        SimpleCommandGroup.__init__(
            self, "joint_head_tilt", range_rad, acceptable_joint_error=0.52, node=node
        )
        if calibrated_offset_rad is None:
            calibrated_offset_rad = node.controller_parameters["tilt_angle_offset"]
        self.calibrated_offset_rad = calibrated_offset_rad
        if calibrated_looking_up_offset_rad is None:
            calibrated_looking_up_offset_rad = node.controller_parameters[
        ...
        if backlash_transition_angle_rad is None:
            backlash_transition_angle_rad = node.controller_parameters[
    ...
    def update_joint_range(self, joint_range, node=None):
        ...
        range_rad = (
            node.robot.head.motors["head_tilt"].ticks_to_world_rad(range_ticks[1]),
    ...
    def init_execution(self, robot, robot_status, **kwargs):
        if self.active:
            ...
            self.looking_up = self.goal["position"] > (
                self.backlash_transition_angle_rad + self.calibrated_offset_rad
            )
    ...
    def update_execution(self, robot_status, **kwargs):
        ...
        if self.active:
            tilt_backlash_correction = (
                self.looking_up_offset_rad if self.looking_up else 0.0
            )
            tilt_current = (
                robot_status["head"]["head_tilt"]["pos"]
                + self.calibrated_offset_rad
                + tilt_backlash_correction
            )
...
class WristYawCommandGroup(SimpleCommandGroup):
    ...
    def update_joint_range(self, joint_range, node=None):
        ...
        range_rad = (
            node.robot.end_of_arm.motors["wrist_yaw"].ticks_to_world_rad(
                range_ticks[1]
            ),
            node.robot.end_of_arm.motors["wrist_yaw"].ticks_to_world_rad(
                range_ticks[0]
    ...
    def update_execution(self, robot_status, **kwargs):
        ...
        if self.active:
            self.error = (
                self.goal["position"] - robot_status["end_of_arm"]["wrist_yaw"]["pos"]
            )
            return self.name, self.error
...
class GripperCommandGroup(SimpleCommandGroup):
    ...
    def update_joint_range(self, joint_range, node=None):
        if joint_range is not None:
            self.range = joint_range
            self.range_aperture_m = (
                self.gripper_conversion.robotis_to_aperture(joint_range[0]),
                self.gripper_conversion.robotis_to_aperture(joint_range[1]),
            )
            self.range_finger_rad = (
                self.gripper_conversion.robotis_to_finger(joint_range[0]),
        ...
        range_rad = (
            node.robot.end_of_arm.motors["stretch_gripper"].ticks_to_world_rad(
                range_ticks[0]
            ),
            node.robot.end_of_arm.motors["stretch_gripper"].ticks_to_world_rad(
                range_ticks[1]
        ...
        self.range_aperture_m = (
            self.gripper_conversion.robotis_to_aperture(range_robotis[0]),
            self.gripper_conversion.robotis_to_aperture(range_robotis[1]),
        )
        self.range_finger_rad = (
            self.gripper_conversion.robotis_to_finger(range_robotis[0]),
    ...
    def update(self, commanded_joint_names, invalid_joints_callback, **kwargs):
        self.active = False
        self.index = None
        active_gripper_joint_names = list(
            set(commanded_joint_names) & set(self.gripper_joint_names)
        )
        if len(active_gripper_joint_names) > 1:
            err_str = (
                "Received a command for the gripper that includes more than one gripper joint name: {0}. "
                ...
                "{1}."
            ).format(active_gripper_joint_names, self.gripper_joint_names)
    ...
    def set_goal(self, point, invalid_goal_callback, fail_out_of_range_goal, **kwargs):
        ...
        if self.active:
            goal_pos = (
                point.positions[self.index]
                if len(point.positions) > self.index
                else None
            )
            if goal_pos is None:
                err_str = (
                    ...
                    "in commanded_joint_names."
                ).format(len(point.positions), self.name, self.index)
            ...
            joint_range = (
                ...
                else self.range_finger_rad
            )
            self.goal["position"] = hm.bound_ros_command(
                joint_range, goal_pos, fail_out_of_range_goal
            )
            self.goal["velocity"] = (
                point.velocities[self.index]
                if len(point.velocities) > self.index
                else None
            )
            self.goal["acceleration"] = (
                point.accelerations[self.index]
                if len(point.accelerations) > self.index
                else None
            )
            self.goal["contact_threshold"] = (
                point.effort[self.index] if len(point.effort) > self.index else None
            )
            if self.goal["position"] is None:
                err_str = (
                    "Received {0} goal point that is out of bounds. "
                    "Range = {1}, but goal point = {2}."
                ).format(self.name, joint_range, goal_pos)
                invalid_goal_callback(err_str)
                return False
    ...
    def init_execution(self, robot, robot_status, **kwargs):
        if self.active:
            ...
            if self.name == "gripper_aperture":
                gripper_robotis_error = self.gripper_conversion.aperture_to_robotis(
                    gripper_error
                )
            elif (self.name == "joint_gripper_finger_left") or (
                self.name == "joint_gripper_finger_right"
            ):
                gripper_robotis_error = self.gripper_conversion.finger_to_robotis(
                    gripper_error
                )
            robot.end_of_arm.move_by(
                "stretch_gripper",
    ...
    def update_execution(self, robot_status, **kwargs):
        ...
        if self.active:
            ...
            if self.name == "gripper_aperture":
                gripper_current = self.gripper_conversion.robotis_to_aperture(
                    robotis_pct
                )
            elif (self.name == "joint_gripper_finger_left") or (
                self.name == "joint_gripper_finger_right"
    ...
    def joint_state(self, robot_status, **kwargs):
        joint_name = (
            kwargs["joint_name"] if "joint_name" in kwargs.keys() else self.name
        )
        gripper_status = robot_status["end_of_arm"]["stretch_gripper"]
        pos_aperture_m, pos_rad, effort, vel = self.gripper_conversion.status_to_all(
            gripper_status
        )
        if joint_name == "gripper_aperture":
            return (pos_aperture_m, vel, effort)
        elif (joint_name == "joint_gripper_finger_left") or (
...
class ArmCommandGroup(SimpleCommandGroup):
    def __init__(self, range_m=None, calibrated_retracted_offset_m=None, node=None):
        SimpleCommandGroup.__init__(
            self, "wrist_extension", range_m, acceptable_joint_error=0.008, node=node
        )
        self.telescoping_joints = [
            "joint_arm_l3",
            "joint_arm_l2",
            "joint_arm_l1",
            "joint_arm_l0",
        ...
        if calibrated_retracted_offset_m is None:
            calibrated_retracted_offset_m = node.controller_parameters[
    ...
    def update(self, commanded_joint_names, invalid_joints_callback, **kwargs):
        ...
        self.is_telescoping = False
        self.index = None
        active_telescoping_joint_names = list(
            set(commanded_joint_names) & set(self.telescoping_joints)
        )
        if self.name in commanded_joint_names:
            if len(active_telescoping_joint_names) == 0:
                ...
                err_str = (
                    "Received a command for the wrist_extension joint and one or more telescoping_joints. "
                    "These are mutually exclusive options. The joint names in the received command = "
                    "{0}"
                ).format(commanded_joint_names)
                invalid_joints_callback(err_str)
                return False
            ...
            if len(active_telescoping_joint_names) == len(self.telescoping_joints):
                ...
                self.index = [
                    commanded_joint_names.index(i) for i in self.telescoping_joints
                ]
            else:
                err_str = (
                    "Commands with telescoping joints requires all telescoping joints to be present. "
                    "Only received {0} of {1} telescoping joints. They are = "
                    "{2}"
                ).format(
                    len(active_telescoping_joint_names),
                    len(self.telescoping_joints),
    ...
    def set_goal(self, point, invalid_goal_callback, fail_out_of_range_goal, **kwargs):
        ...
        if self.active:
            if self.is_telescoping:
                goal_pos = (
                    sum([point.positions[i] for i in self.index])
                    if len(point.positions) > max(self.index)
                    else None
                )
                self.goal["velocity"] = (
                    point.velocities[self.index[0]]
                    if len(point.velocities) > self.index[0]
                    else None
                )
                self.goal["acceleration"] = (
                    point.accelerations[self.index[0]]
                    if len(point.accelerations) > self.index[0]
                    else None
                )
                self.goal["contact_threshold"] = (
                    point.effort[self.index[0]]
                    if len(point.effort) > self.index[0]
                    else None
                )
            else:
                goal_pos = (
            ...
            if self.goal["position"] is None:
                err_str = (
                    "Received {0} goal point that is out of bounds. "
                    "Range = {1}, but goal point = {2}."
                ).format(self.name, self.range, goal_pos)
                invalid_goal_callback(err_str)
                return False
    ...
    def init_execution(self, robot, robot_status, **kwargs):
        if self.active:
            ...
            robot.arm.move_by(
                ...
                if self.goal["contact_threshold"] is not None
                else None,
            )
    ...
    def update_execution(self, robot_status, **kwargs):
        success_callback = (
            kwargs["success_callback"] if "success_callback" in kwargs.keys() else None
        )
        self.error = None
        if self.active:
            ...
            return (
                (self.telescoping_joints, self.error)
                if self.is_telescoping
                else (self.name, self.error)
            )
...
class MobileBaseCommandGroup(SimpleCommandGroup):
    ...
    def update(self, commanded_joint_names, invalid_joints_callback, **kwargs):
        ...
        active_incrementing_joint_names = list(
            set(commanded_joint_names) & set(self.incrementing_joint_names)
        )

        if robot_mode == "manipulation":
            err_str = (
                "MobileBaseCommandGroup.update: Manipulation mode is no longer allowed."
            )
            invalid_joints_callback(err_str)
            return False
        ...
        if len(active_incrementing_joint_names) != 0:
            if robot_mode == "position":
                ...
                if "translate_mobile_base" in active_incrementing_joint_names:
                    ...
                    self.index_translate_mobile_base = commanded_joint_names.index(
                        "translate_mobile_base"
                    )
                if "rotate_mobile_base" in active_incrementing_joint_names:
                    self.active_rotate_mobile_base = True
                    self.index_rotate_mobile_base = commanded_joint_names.index(
                        "rotate_mobile_base"
                    )
            else:
                err_str = (
                    "MobileBaseCommandGroup.update: Must be in position mode to receive a command for the {0} joint(s). "
                    "Current mode = {1}."
                ).format(active_positioning_joint_names, robot_mode)
                invalid_joints_callback(err_str)
                return False
    ...
    def set_goal(self, point, invalid_goal_callback, fail_out_of_range_goal, **kwargs):
        ...
        self.goal_translate_mobile_base = {
            "position": None,
            "velocity": None,
            "acceleration": None,
            "contact_threshold": None,
        }
        self.goal_rotate_mobile_base = {
            "position": None,
            "velocity": None,
            "acceleration": None,
            "contact_threshold": None,
        ...
        if self.active:
            ...
            if self.active_translate_mobile_base:
                if len(point.positions) <= self.index_translate_mobile_base:
                    err_str = (
                        "MobileBaseCommandGroup.set_goal: Received goal point with positions array "
                ...
                if not np.isclose(
                    point.positions[self.index_translate_mobile_base],
                    0.0,
                    rtol=1e-5,
                    atol=1e-8,
                    equal_nan=False,
                ):
                    self.goal_translate_mobile_base["position"] = point.positions[
                        self.index_translate_mobile_base
                    ]
                    self.goal_translate_mobile_base["velocity"] = (
                        point.velocities[self.index_translate_mobile_base]
                        if len(point.velocities) > self.index_translate_mobile_base
                        else None
                    )
                    self.goal_translate_mobile_base["acceleration"] = (
                        point.accelerations[self.index_translate_mobile_base]
                        if len(point.accelerations) > self.index_translate_mobile_base
                        else None
                    )
                    self.goal_translate_mobile_base["contact_threshold"] = (
                        point.effort[self.index_translate_mobile_base]
                        if len(point.effort) > self.index_translate_mobile_base
                        else None
                    )
            ...
            if self.active_rotate_mobile_base:
                ...
                if not np.isclose(
                    point.positions[self.index_rotate_mobile_base],
                    0.0,
                    rtol=1e-5,
                    atol=1e-8,
                    equal_nan=False,
                ):
                    self.goal_rotate_mobile_base["position"] = point.positions[
                        self.index_rotate_mobile_base
                    ]
                    self.goal_rotate_mobile_base["velocity"] = (
                        point.velocities[self.index_rotate_mobile_base]
                        if len(point.velocities) > self.index_rotate_mobile_base
                        else None
                    )
                    self.goal_rotate_mobile_base["acceleration"] = (
                        point.accelerations[self.index_rotate_mobile_base]
                        if len(point.accelerations) > self.index_rotate_mobile_base
                        else None
                    )
                    self.goal_rotate_mobile_base["contact_threshold"] = (
                        point.effort[self.index_rotate_mobile_base]
                        if len(point.effort) > self.index_rotate_mobile_base
                        else None
                    )
    ...
    def init_execution(self, robot, robot_status, **kwargs):
        ...
        if self.active:
            if self.active_translate_mobile_base or self.active_rotate_mobile_base:
                (_, mobile_base_error_m), (
                    _,
                    mobile_base_error_rad,
                ) = self.update_execution(robot_status)
                if mobile_base_error_m is not None:
                    robot.base.translate_by(
                        mobile_base_error_m,
                        v_m=self.goal_translate_mobile_base["velocity"],
                        a_m=self.goal_translate_mobile_base["acceleration"],
                        contact_thresh=self.goal_translate_mobile_base[
                            "contact_threshold"
                        ],
                    )
                    ...
                    robot.base.rotate_by(
                        ...
                        contact_thresh=self.goal_rotate_mobile_base[
                            "contact_threshold"
                        ],
                    )
            else:
        ...
        if self.active:
            if self.active_translate_mobile_base or self.active_rotate_mobile_base:
                if self.goal_translate_mobile_base["position"] is not None:
                    if (
                        robot_status["base"]["left_wheel"]["in_guarded_event"]
                        or robot_status["base"]["right_wheel"]["in_guarded_event"]
                    ) and success_callback:
                        success_callback("translate_mobile_base contact detected.")
                        return True
                    dist = np.sqrt(
                        np.square(currx - self.startx) + np.square(curry - self.starty)
                    )
                    self.error_translate_mobile_base_m = (
                        self.goal_translate_mobile_base["position"]
                        - (dist * np.sign(self.goal_translate_mobile_base["position"]))
                    )
                    return [
                        ("translate_mobile_base", self.error_translate_mobile_base_m),
                        ("rotate_mobile_base", self.error_rotate_mobile_base_rad),
                    ]
                    ...
                    self.error_rotate_mobile_base_rad = hm.angle_diff_rad(
                        self.goal_rotate_mobile_base["position"], rot
                    )
                    return [
                        ("translate_mobile_base", self.error_translate_mobile_base_m),
                        ("rotate_mobile_base", self.error_rotate_mobile_base_rad),
    ...
    def goal_reached(self):
        if self.active:
            if self.active_translate_mobile_base or self.active_rotate_mobile_base:
                if self.active_translate_mobile_base:
                    reached = (
                        abs(self.error_translate_mobile_base_m)
                        < self.acceptable_mobile_base_error_m
                    )
                    if not (
                        abs(self.error_translate_mobile_base_m)
                        ...
                        speed = np.sqrt(
                            np.square(self.base_status["x_vel"])
                            + np.square(self.base_status["y_vel"])
                        )
                        reached = reached and (speed < self.min_m_per_s)
                    return reached
                elif self.active_rotate_mobile_base:
                    reached = (
                        abs(self.error_rotate_mobile_base_rad)
                        < self.acceptable_mobile_base_error_rad
                    )
                    if not (
                        abs(self.error_rotate_mobile_base_rad)
                        < self.excellent_mobile_base_error_rad
                    ):
                        # Use velocity to help decide when the low-level command has been finished
