...
class JointTrajectoryAction:
    def __init__(self, node):
        ...
        self.head_pan_cg = (
            HeadPanCommandGroup(node=self.node)
            if "head_pan" in self.node.robot.head.joints
            else None
        )
        self.head_tilt_cg = (
            HeadTiltCommandGroup(node=self.node)
            if "head_tilt" in self.node.robot.head.joints
            else None
        )
        self.wrist_yaw_cg = (
            WristYawCommandGroup(node=self.node)
            if "wrist_yaw" in self.node.robot.end_of_arm.joints
            else None
        )
        self.gripper_cg = (
            GripperCommandGroup(node=self.node)
            if "stretch_gripper" in self.node.robot.end_of_arm.joints
            else None
        )
        ...
        for joint in self.node.robot.end_of_arm.joints:
            module_name = self.node.robot.end_of_arm.params["devices"][joint].get(
                "ros_py_module_name"
            )
            class_name = self.node.robot.end_of_arm.params["devices"][joint].get(
                "ros_py_class_name"
            )
            if module_name and class_name:
                endofarm_cg = getattr(importlib.import_module(module_name), class_name)(
                    node=self.node
                )
    ...
    def execute_cb(self, goal):
        ...
        rospy.loginfo(
            (
                "{0} joint_traj action: New trajectory received with joint_names = "
                "{1}"
            ).format(self.node.node_name, commanded_joint_names)
        )
        ...
        num_valid_points = sum(
            [c.get_num_valid_commands() for c in self.command_groups]
        )
        if num_valid_points <= 0:
            err_str = (
                "Received a command without any valid joint names."
                "Received joint names = {0}"
            ...
            err_str = (
                ...
                num_valid_points, len(commanded_joint_names), commanded_joint_names
            )
            self.invalid_joints_callback(err_str)
            self.node.robot_mode_rwlock.release_read()
            return
        ...
        for pointi, point in enumerate(goal.trajectory.points):
            rospy.logdebug(
                ("{0} joint_traj action: " "target point #{1} = <{2}>").format(
                    self.node.node_name, pointi, point
                )
            )

            valid_goals = [
                c.set_goal(
                    point, self.invalid_goal_callback, self.node.fail_out_of_range_goal
                )
                for c in self.command_groups
            ]
            ...
            while not all(goals_reached):
                if (
                    rospy.Time.now() - goal_start_time
                ) > self.node.default_goal_timeout_duration:
                    err_str = (
                        "Time to execute the current goal point = <{0}> exceeded the "
                        "default_goal_timeout = {1}"
                    ).format(point, self.node.default_goal_timeout_s)
                ...
                with self.node.robot_stop_lock:
                    if self.node.stop_the_robot or self.server.is_preempt_requested():
                        rospy.logdebug(
                            (
                                "{0} joint_traj action: PREEMPTION REQUESTED, but not stopping "
                                "current motions to allow smooth interpolation between "
                                "old and new commands."
                            ).format(self.node.node_name)
                        )
                ...
                named_errors = [
                    c.update_execution(
                        robot_status, success_callback=self.success_callback
                    )
                    for c in self.command_groups
                ]
            ...
            rospy.logdebug(
                "{0} joint_traj action: Achieved target point.".format(
                    self.node.node_name
                )
            )
    ...
    def invalid_joints_callback(self, err_str):
        if self.server.is_active() or self.server.is_preempt_requested():
            rospy.logerr(
                "{0} joint_traj action: {1}".format(self.node.node_name, err_str)
            )
    ...
    def feedback_callback(self, commanded_joint_names, desired_point, named_errors):
        ...
        rospy.logdebug(
            "{0} joint_traj action: sending feedback".format(self.node.node_name)
        )
    ...
    def success_callback(self, success_str):
        rospy.loginfo(
            "{0} joint_traj action: {1}".format(self.node.node_name, success_str)
        )
