...
class BCModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.model = create_mlp(3, 14, [256, 256], [0.10, 0.10], nn.GELU)
...
class HalSkillsNode(hm.HelloNode):
    def __init__(self):
        ...
        pth_path = "/home/strech/catkin_ws/src/stretch_ros/stretch_learning/checkpoints/bc/state_dict_bc_gelu_epoch_40_loss_1.8893291793132005e-05.pt"
    ...
    def get_command(self, pred):
        ...
        for i,joint in enumerate(list(pred)):
            if joint > 1e-3:
                commands.append({"joint": joint_names[i], "delta": joint})
    ...
    def send_command(self, commands):
        ...
        if (joint_state is not None) and (commands is not None):
    ...
    def main(self):
        ...
        while not rospy.is_shutdown():
            ...
            if (
                ...
                in_y = (
                    np.abs(end_eff_tensor[1] - local_goal_pos[1]) < 0.04
                ) or deep_y  # 0.01
