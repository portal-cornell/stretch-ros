...
class HalSkillsPlace(hm.HelloNode):
    def __init__(self, goal_pos):
        ...
        self.goal_point = torch.Tensor(self.goal_pos).to(device)

        pth_path = "/home/strech/new_ws/src/stretch_ros/stretch_learning/checkpoints/ppo_point_and_shoot/policy_fixed_reduced.pth"
        # pth_path = "/home/strech/new_ws/src/stretch_ros/stretch_learning/checkpoints/ppo_point_and_shoot/policy_base.pth"
    ...
    def save_traj(self, dist_to_goal, time_to_goal, filename):
        with open(filename, 'wb') as f:
            pickle.dump((dist_to_goal, time_to_goal), f)
    ...
    def main(self, reset=False):
        ...
        while not rospy.is_shutdown():
            ...
            if (
                ...
                aruco_point_msg = PointStamped()
                aruco_point_msg.header.frame_id = "aruco_frame_depth"
                aruco_point_msg.point.x = fixed_goal[0]
                aruco_point_msg.point.y = fixed_goal[1]
                aruco_point_msg.point.z = fixed_goal[2]

                try:
                    trans_point_msg = self.listener_tf.transformPoint(
                        "base_link", aruco_point_msg
                    )
                except (
                    tf2_ros.LookupException,
                    tf2_ros.ConnectivityException,
                ...
                local_goal_pos = torch.tensor(
                    [
                        trans_point_msg.point.x, 
                        trans_point_msg.point.y, 
                        trans_point_msg.point.z,
                    ]
                )
                ...
                dist_to_goal.append(np.linalg.norm(delta))
                time_to_goal.append(time.time() - start_time)
                ...
                in_y = np.abs(local_goal_pos[1] - end_eff_tensor[1]) < 0.012
                in_x = np.abs(local_goal_pos[0] - end_eff_tensor[0]) < 0.007  # 0.012
                full_extend = self.joint_states.position[wrist_idx] > 0.42
        ...
        print(dist_to_goal)
        print(time_to_goal)
        self.save_traj(dist_to_goal, time_to_goal, "./traj2.pkl")
