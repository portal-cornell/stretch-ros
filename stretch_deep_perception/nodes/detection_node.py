...
class DetectionNode:
    def __init__(self, detector, default_marker_name, node_name,
        ...
        self.marker_collection = dr.DetectionBoxMarkerCollection(
            default_marker_name)
    ...
    def image_callback(self, ros_rgb_image, ros_depth_image, rgb_camera_info):
        ...
        if time_diff > 0.0001:
            print(
                'WARNING: The rgb image and the depth image were not taken at the same time.')
            print('         The time difference between their timestamps =',
                  closest_time_diff, 's')

        # Rotate the image by 90deg to account for camera
        # orientation. In the future, this may be performed at the
        # image source.
        detection_box_image = cv2.rotate(
            self.rgb_image, cv2.ROTATE_90_CLOCKWISE)
        ...
        detections_3d = d2.detections_2d_to_3d(detections_2d, self.rgb_image, self.camera_info, self.depth_image,
    ...
    def add_to_point_cloud(self, x_mat, y_mat, z_mat, mask):
        points = [[x, y, z] for x, y, z, m in zip(
            x_mat.flatten(), y_mat.flatten(), z_mat.flatten(), mask.flatten()) if m > 0]
    ...
    def publish_image(self, image):
        br = CvBridge()
        self.visualize_img_pub.publish(br.cv2_to_imgmsg(image, "bgr8"))
    ...
    def main(self):
        ...
        self.rgb_image_subscriber = message_filters.Subscriber(
            self.rgb_topic_name, Image)

        self.depth_topic_name = '/camera/aligned_depth_to_color/image_raw'
        self.depth_image_subscriber = message_filters.Subscriber(
            self.depth_topic_name, Image)

        self.camera_info_subscriber = message_filters.Subscriber(
            '/camera/color/camera_info', CameraInfo)

        self.synchronizer = message_filters.TimeSynchronizer(
            [self.rgb_image_subscriber, self.depth_image_subscriber, self.camera_info_subscriber], 10)
        self.synchronizer.registerCallback(self.image_callback)

        self.visualize_markers_pub = rospy.Publisher(
            '/' + self.topic_base_name + '/marker_array', MarkerArray, queue_size=1)
        self.visualize_axes_pub = rospy.Publisher(
            '/' + self.topic_base_name + '/axes', MarkerArray, queue_size=1)
        self.visualize_point_cloud_pub = rospy.Publisher(
            '/' + self.topic_base_name + '/point_cloud2', PointCloud2, queue_size=1)
        self.visualize_img_pub = rospy.Publisher(
            '/' + self.topic_base_name + '/image', Image, queue_size=1)
